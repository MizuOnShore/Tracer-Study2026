...
| PRED-01 | Pathway end-to-end | Valid batch → probabilities → meta → immutable run → aggregate UI | Inference-only upload, shared validation, server batch proxy, atomic run persistence, summary/drill-down | prediction validation/execute routes; `service.py`; run migration; results dashboard | Implemented in code | Critical | Apply migration, redeploy Render, verify with active artifact |
| PRED-02 | NEET end-to-end | Valid batch → LR probability/threshold → saved run → aggregate UI | Same batch receives separate NEET inference; predicted NEET is explicitly separated from pathway output | execute route; `service.py`; results dashboard | Implemented in code | Critical | Verify real threshold/output against active artifact |
| PRED-03 | Prediction/actual separation | Inference uploads never become actual respondent records | Dedicated `prediction-inputs`, runs, staging and issues; no `respondent_records` write | migration; prediction validation route | Implemented | Critical | Retain this boundary |
| PRED-04 | Prediction history | Completed runs remain independently reproducible | Run parent, two model results per valid source row, model versions and summary persisted | `prediction_runs`; `prediction_results`; history/results pages | Implemented in code | High | Establish retention/deletion policy |
| DISC-01 | Run-based discussion | Select a saved run and calculate supported, non-causal findings | Discussion has no upload; it derives overview, findings, attention groups, reliability and considerations from selected result rows | `app/insights/page.tsx`; prediction analytics helpers | Implemented in code | High | Validate wording with adviser using real results |
