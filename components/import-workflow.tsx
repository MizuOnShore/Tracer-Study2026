...
export function ImportWorkflow() {
  ...
  const [committedRecords, setCommittedRecords] = useState<number | null>(null)
  const [busy, setBusy] = useState(false)

  async function validate(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()
    setBusy(true); setMessage(""); setPreview(null); setCommittedRecords(null)
  ...
  async function commit() {
    ...
    else { setMessage(`${result.committed_records} records committed successfully.`); setCommittedRecords(Number(result.committed_records)); setPreview(null); router.refresh() }
    setBusy(false)
  }

  return <>
    <section className="panel"><h2>Upload tracer file</h2><p>CSV or XLSX only, maximum 10 MB. Uploading validates and stages the file; dashboard charts update only after you commit all validated rows.</p><form onSubmit={validate}><div className="field"><label className="required" htmlFor="import-file">Tracer data file</label><input id="import-file" name="file" type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required /></div><div className="form-actions"><button className="button" disabled={busy} type="submit">{busy ? "Checking file…" : "Upload and validate"}</button><a className="button secondary" href="/api/imports/template">Download column template</a></div></form></section>
    {message && <div className={`notice ${committedRecords !== null ? "success" : "error"}`} role="status"><span>{message}</span>{committedRecords !== null && <Link className="button small" href="/dashboard">View updated dashboard</Link>}</div>}
    {preview && <section className="panel"><h2>Validation preview</h2><div className="metric-grid"><Metric label="Rows" value={preview.total_rows} /><Metric label="Valid" value={preview.valid_rows} /><Metric label="Invalid" value={preview.invalid_rows} /><Metric label="Issues shown" value={preview.issues.length} /></div><div className="notice info"><strong>Validation is not the final import.</strong> Review the results, then select <strong>Commit validated records</strong> to persist the rows and update dashboard charts.</div>{preview.issues.length > 0 && <div className="table-wrap"><table><thead><tr><th>Row</th><th>Column</th><th>Severity</th><th>Issue</th></tr></thead><tbody>{preview.issues.map((issue, index) => <tr key={`${issue.code}-${issue.row_number}-${index}`}><td>{issue.row_number ?? "File"}</td><td>{issue.column_name ?? "—"}</td><td><span className={`badge ${issue.severity === "error" ? "error" : "warning"}`}>{issue.severity}</span></td><td>{issue.message}</td></tr>)}</tbody></table></div>}<div className="form-actions"><button className="button" type="button" disabled={busy || preview.status !== "validated" || preview.invalid_rows > 0} onClick={commit}>Commit validated records</button>{preview.invalid_rows > 0 && <span className="privacy-note">Commit is disabled because one or more rows are invalid. Correct the source file and upload it again.</span>}</div></section>}
  </>
}
