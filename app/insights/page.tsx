...
export default async function InsightsPage() {
  ...
  return <AppShell profile={profile}><header className="page-header"><div><span className="eyebrow">Interpretation workspace</span><h1>Discussion and insights</h1><p>Findings are labeled by evidence type. Descriptive records, model outputs, and recommendations are not presented as causal conclusions.</p></div></header>{error ? <StatusPanel title="Insights unavailable" description="The aggregate source query failed." code="ANALYTICS_QUERY_FAILED" /> : total === 0 ? <StatusPanel title="No evidence available for discussion" description="Insights are withheld until real, validated tracer records are persisted." code="DATA_NOT_AVAILABLE" /> : <>
    <section className="panel"><span className="badge">Descriptive finding</span><h2 style={{ marginTop: 12 }}>Recorded primary status</h2><p>Among {total} persisted respondents, the largest recorded primary-status group is <strong>{leading[0].replaceAll("_", " ")}</strong> with {leading[1]} respondent{leading[1] === 1 ? "" : "s"} ({((leading[1] / total) * 100).toFixed(1)}%). This describes the available respondent data and does not establish a population rate or a cause.</p></section>
    <section className="panel"><span className="badge">Descriptive finding</span><h2 style={{ marginTop: 12 }}>Preparation ratings</h2><p>The mean subject-relevance rating is <strong>{Number(overview?.average_subject_relevance ?? 0).toFixed(2)}/5</strong>, and the mean preparedness rating is <strong>{Number(overview?.average_preparedness ?? 0).toFixed(2)}/5</strong>. These are self-reported ratings from available records.</p></section>
    ...
    <section className="panel"><span className="badge warning">Recommendation</span><h2 style={{ marginTop: 12 }}>School review priorities</h2><p>{Number(overview?.neet ?? 0) > 0 ? `Review support needs and barriers reported by the ${Number(overview?.neet)} respondent(s) currently recorded as NEET, while protecting individual confidentiality.` : "Continue collecting representative tracer responses before prioritizing interventions from these data."} This is a decision-support recommendation, not a causal finding.</p></section>
