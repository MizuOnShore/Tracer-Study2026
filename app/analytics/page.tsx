...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<{ year?: string; strand?: string; status?: string }> }) {
  ...
  return <AppShell profile={profile}>
    <header className="page-header"><div><span className="eyebrow">Imported descriptive analytics</span><h1>Graduate outcomes analytics</h1><p>Filters are applied to committed import aggregates. Website survey records are not an active data source.</p></div></header>
    ...
    {error ? <StatusPanel title="Analytics unavailable" description="The import aggregate query failed. Check that the database views in the Supabase migration were applied." code="ANALYTICS_QUERY_FAILED" /> : rows.length === 0 ? <StatusPanel title="No results for these filters" description="No committed import records match the current year, strand, and status filters." code="DATA_NOT_AVAILABLE" /> : <section className="panel"><h2>Imported aggregate source table</h2><p>This table is the verified fallback and reconciliation source for Power BI.</p><div className="table-wrap"><table><thead><tr><th>Graduation year</th><th>Strand</th><th>Primary status</th><th>Respondents</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.graduation_year}-${row.strand}-${row.current_status}`}><td>{row.graduation_year}</td><td>{row.strand}</td><td>{row.current_status.replaceAll("_", " ")}</td><td>{Number(row.respondent_count)}</td></tr>)}</tbody></table></div></section>}
